...
BulkHtmlLoader.prototype = {
	...
	load: function(queue, callback) {
		...
		if(this._total > 0){
            throw new Error('Load can only be called once per instance. To load more queues create a new HtmlLoader instance');
		}
    ...
    /**
     * @returns {{loaded: number, total: (number|Number|*)}}
     */
     ...
     */
	toString: function(){
		return '[Object HtmlLoader], total: ' + this._queue.length + ', loaded: ' + this._loaded + ', errors: ' + this._numErrors;
	},
	...
	_loadItem: function(loaderItem, finalCallback){
        ...
        /**
         * Handles a single http request once
         * @param loaderItem
         * @param retryCallback
         */
		var loadWithRetry = function(loaderItem, retryCallback){
			...
			request(options, function(error, response, body) {
                ...
                // Callback has already been fired at this stage
				if(hasTimedOut){
                    return;
				}
