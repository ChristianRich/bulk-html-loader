...
LoaderItem.prototype = {
	...
	/**
	 * Returns a String representation of a LoaderItem instance and displays information about it's current status, system memory usage and more
     * @return {string}
	 */
	toString: function(){

        if(this.getError()){
            return '[LoaderItem] ' + this.getStatus() + ' ' + this.getError().code + ' ' + this.getError().errno + ' ' + this.getUrl() + ', ' + util.system.getMemUsage(0, true);
        }

        return '[LoaderItem] ' + this.getStatus() + ' ' + this.getUrl() + ', ' + util.system.getMemUsage(0, true);
	}
};
